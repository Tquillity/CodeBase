...
def generate_list_content(gui, files_to_copy, repo_path, lock, completion_callback, content_cache, list_read_errors):
    ...
    thread = threading.Thread(target=generate_content, args=(files_to_copy, repo_path, lock, wrapped_completion, content_cache, list_read_errors, None, gui), daemon=True)
    thread.start()
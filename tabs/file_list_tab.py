...
class FileListTab(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.load_list_button = self.gui.create_button(self, "Load List", None, "Load pasted file list for selection")
        self.load_list_button.pack(side=tk.LEFT, padx=10, pady=5)
        self.copy_list_button = self.gui.create_button(self, "Copy from List", None, "Copy content from listed files", state=tk.DISABLED)
    ...
    def copy_from_list(self):
        ...
        generate_list_content(self.gui.list_selected_files, self.gui.current_repo_path, self.gui.file_handler.lock,
